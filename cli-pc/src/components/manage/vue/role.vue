  ...
  <div>
    <div>
      <div class="a-admin-head">
        <el-button type="primary" @click="openAddRole" style="margin-right:15px;">新增</el-button>
    ...
    <div>
      <el-dialog title="关联菜单" :visible.sync="dialogMenuVisible">
        <el-tree :data="menuListData" show-checkbox :props="roleMenuProps" default-expand-all
            node-key="id" :default-checked-keys="menuCheckData" ref="menuTree">
        </el-tree>
        <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    delRole(id) {
      this.$confirm('此操作将永久删除该角色及其关联数据，是否继续？', '提示', {
        ...
        requestDelRole({ id: id }).then(data => {
          var _code = data.code;
          if (_code == 100) {
            this.$message({
              type: 'success',
