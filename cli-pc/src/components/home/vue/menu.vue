        ...
        <div class="tags-box">
          <div class="tags" v-for="(item,index) in tagsList">
            <p style="display:inline-block">
              <span class="ivu-tag-dot-inner" :style="tagsIndex==index+1?'background:#2d8cf0':'background:#e9eaec'"></span>
              <span v-text="item.name"></span>
            </p>
            <i class="iconfont gt-delete"></i>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      '$route' (to, from) {
        console.log(to, from)
        console.log(this.tagsList, 'tagsList', this.tagsList.length, 'length')
        this.tagsList.push(to)
        this.tagsIndex = this.tagsList.length
      }
    },
    methods: {
      setCollapse() {
        if (this.isCollapse) {
          ...
          this.menuLeftStyle.width = '60px'
          this.menuTopStyle.paddingLeft = '60px'
          this.menuRightStyle.marginLeft = '60px'
