...
public class SwaggerConfig {
    // 扫描包
	private static final String  BASEPACKAGE         = "com.gt.inside.controller";
    // 标题
    private static final String  TITLE               = "Inside API";
    // 描述
    private static final String  DESC                = "多粉内部项目API";
    // 版本
    ...
    public Docket createRestApi() {
	    return new Docket( DocumentationType.SWAGGER_2 ).apiInfo( apiInfo() ).tags( new Tag( "inside", "多粉内部项目API" ), getTags() ).select()
			.apis( RequestHandlerSelectors.basePackage( BASEPACKAGE ) ).paths( PathSelectors.any() ).build();
    }

	private Tag[] getTags() {
        Tag[] tags = { new Tag( "inside", "多粉内部项目相关接口" ) };
        return tags;
    }
